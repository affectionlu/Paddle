...
void ClipByNormKernel(const Context* dev_ctx,
  ...
  auto x = paddle::framework::EigenVector<dtype::float16>::Flatten(*input);
  auto out = paddle::framework::EigenVector<dtype::float16>::Flatten(*output);
...
PD_REGISTER_KERNEL(
    clip_by_norm, GPU, ALL_LAYOUT, phi::ClipByNormFunctor, float) {}
